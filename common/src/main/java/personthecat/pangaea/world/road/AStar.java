...
public class AStar {
    ...
    protected final WeightFunction weight;
    protected Point src;
    protected Point found;

    public AStar(NoiseGraph graph, WeightFunction weight) {
    ...
    public final List<Point> search(PangaeaContext ctx, Point src, Destination dest) {
        if (!Cfg.debugBranches()) {
            return this.doSearch(ctx, src, dest);
        }
        sw.logStart("generating road: %s -> %s", src, dest);
        final List<Point> path = this.doSearch(ctx, src, dest);
        if (path != null) {
    ...
    protected List<Point> doSearch(PangaeaContext ctx, Point src, Destination dest) {
        this.src = src;
        int x = src.x;
        int z = src.z;
        ...
        while (this.hasNext() && len++ < Road.MAX_LENGTH) {
            ...
            if (this.checkDirection(ctx, dest,2, x, z, x - 2, z)
                    || this.checkDirection(ctx, dest, 2, x, z, x + 2, z)
                    || this.checkDirection(ctx, dest, 2, x, z, x, z + 2)
                    || this.checkDirection(ctx, dest, 2, x, z, x, z - 2)
                    || this.checkDirection(ctx, dest, 2.83, x, z, x - 2, z + 2)
                    || this.checkDirection(ctx, dest, 2.83, x, z, x - 2, z - 2)
                    || this.checkDirection(ctx, dest, 2.83, x, z, x + 2, z + 2)
                    || this.checkDirection(ctx, dest, 2.83, x, z, x + 2, z - 2)) {
    ...
    private boolean checkDirection(PangaeaContext ctx, Destination dest, double d, int pX, int pZ, int x, int z) {
        ...
        final double weight = this.weight.compute(ctx, pos);
        if (weight > 100) {
            return false;
        }
