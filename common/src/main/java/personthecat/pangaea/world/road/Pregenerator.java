...
public abstract class Pregenerator {
  ...
  public final void run(final Sampler sampler, final short x, final short y) {
    this.sw.logStart("pre-generating roads...");
    this.generateRegions(sampler, x, y);
  }

  protected abstract void generateRegions(final Sampler sampler, final short x, final short y);
  ...
  public static class SingleThreaded extends Pregenerator {
    ...
    @Override
    protected void generateRegions(final Sampler sampler, final short x, final short z) {
      ...
      for (final Point o : origins) {
        if (this.map.loadRegionFromDisk((short) o.x, (short) o.z) == null) {
          this.map.generateRegion(sampler, (short) o.x, (short) o.z);
          count++;
        }
      }
  ...
  public static class Multithreaded extends Pregenerator {
    ...
    @Override
    protected void generateRegions(final Sampler sampler, final short x, final short y) {
        try (ExecutorService executor = Executors.newFixedThreadPool(Cfg.pregenThreadCount())) {
            ...
            final List<Future<Void>> futures = this.getSortedOrigins(x, y).stream()
                .map(o -> executor.<Void>submit(() -> {
                    if (this.map.loadRegionFromDisk((short) o.x, (short) o.z) == null) {
                        this.map.generateRegion(generators.get(), sampler, (short) o.x, (short) o.z, false);
