...
public record FeatureInjector(
        ...
        @Nullable InjectionMap<List<PlacementModifier>> placement) implements Injector {
    ...
    public static final MapCodec<FeatureInjector> CODEC = codecOf(
        defaulted(BiomePredicate.CODEC, "biomes", BiomePredicate.ALL_BIOMES, FeatureInjector::biomes),
        defaulted(Decoration.CODEC, "step", Decoration.RAW_GENERATION, FeatureInjector::step),
        union(FeatureCodecs.FLAT_CONFIG, FeatureInjector::feature),
        nullable(PLACEMENT_CODEC, "placement", FeatureInjector::placement),
        FeatureInjector::new
    );
    ...
    public void inject(ResourceKey<Injector> key, InjectionContext ctx) {
        ...
        final var configuredHolder = Registry.registerForHolder(configuredRegistry, key.location(), this.feature);

        final var features = this.createFeatures(key, ctx, configuredHolder);
    ...
    private List<Holder<PlacedFeature>> createFeatures(
        ...
        if (this.placement != null && !this.placement.isEmpty()) {
            ...
            list.add(Registry.registerForHolder(placedRegistry, key.location(), placed));
        }
        return list;
    }

    private List<PlacementModifier> addBiomeFilter(List<PlacementModifier> modifiers) {
        if (modifiers.stream().anyMatch(FeatureInjector::isBiomeFilter)) return modifiers;
